...
#define MAX 109
vector<vi> graph(MAX,vi(MAX));
vi block(MAX,-1) , visit(MAX,-1);
int n;
...
int main() {
	...
	rep(i,n) {
		rep(j,n) {
			temp[j].first = graph[i][j] ;
			temp[j].second = j+1;
		}
		sort(all(temp));
		rep(j,n)
			male[i+1][j] = temp[j].second;
	}
	rep(j,n) {
		rep(i,n) {
			temp[i].first = graph[i][j] ;
			temp[i].second = i+1;
		}
		sort(all(temp),cmp);
		rep(i,n)
			female[j+1][i] = temp[i].second;
	}
/*	rep(i,n){	rep(j,n) cout << male[i][j] << " " ;
		cout << endl;
	}
	
	rep(i,n){	rep(j,n) cout << female[i][j] << " " ;
		cout << endl;
	}
*/
	stable_marriage() ;
