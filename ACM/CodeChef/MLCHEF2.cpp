...
void update(int num, int i, int j, int a, int b, int x) {
	...
	if ( i >= a && j <= b ) {
		if ( i == j ) {
			st[num].decrement  += x ;
			st[num].least_healthy -= st[num].decrement ;
			if(st[num].least_healthy <= 0) {
...
int query (int num, int i, int j, int a, int b,int dec = 0) {
 	if ( j < a || i > b || j < i)
		return 0 ;
	if ( i >= a && j <= b ) {
		if(i==j) {
			if (st[num].least_healthy - dec <= 0) {
				st[num].least_healthy = imax ;
		...
		if( st[num].least_healthy - dec <= 0 ) {
			return query(2*num+1,i,(i+j)/2,a,b,dec+st[num].decrement) + query(2*num+2,(i+j)/2+1,j,a,b,dec+st[num].decrement);
		}
		else
			return st[num].alive_count ;
	}
